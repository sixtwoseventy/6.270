...
float bounds[4] = {-2048, 2048, -2048, 2048};
...
void shuffle(){
    ...
    unsigned int * buffer =malloc(len);
    fread(buffer,len,1,urandom);
    srand(*buffer);
    printf("random seed: %u", *buffer);
    free(buffer);
...
void updateHUD(IplImage *out) {
    ...
    if (!warpDisplay) {
        ...
        sprintf(buf, "Score: %u",  gameData.coords[1].score);
        centeredFitTitleText(out, CV_RGB(255,255,255), 450, 250, buf);
         
        if (gameData.coords[0].id<MAX_ROBOT_ID){
            sprintf(buf, "Team: %u",  gameData.coords[0].id);
            centeredFitTitleText(out, CV_RGB(255,255,255), 350, 250, buf);
        }
...
/*
    if ((fd_tx = open("tx", "w")) < 0 ) {
        fprintf(stderr, "fifo broken\n");
    }
//for a rainy day, prints the rf data to a fifo instead which can be read from in a terminal
	
    dprintf(fd_tx, "Heloooo\n");
   */
    serial_sync(fd_tx);
...
void targetGame(IplImage *out) {
    if ((matchState == MATCH_RUNNING)&&(gameData.coords[0].id<30)){
        ...
        if ((50000>((gameData.coords[1].x-gameData.coords[0].x)*(gameData.coords[1].x-gameData.coords[0].x)+((gameData.coords[1].y-gameData.coords[0].y)*(gameData.coords[1].y-gameData.coords[0].y))))&&(pointReached==0)&&((timeNow() - matchStartTime) >= 3)){
            //close enough, flag point reached!
           
            gameData.coords[1].score++;
            //printf("%lu\n",gameData.coords[1].score);
            pointReached = 1;
        }
        if(pointReached == 1){
            //generate point, write to objects, different 
            //coordinate systems!
            //draw circle at point
            shuffle();//shuffles the random number library so we don't give every team the same set of points
            randomX = rand()%(1800+1800+1)-1800; 
            randomY = rand()%(1800+1800+1)-1800;           
            gameData.coords[1].id = 35;//arbitrary
            gameData.coords[1].x = randomX;
            gameData.coords[1].y = randomY;            

            pointReached = 0; 
        }
        
            CvPoint corner = cvPoint(randomX,randomY);
            CvPoint2D32f display_corner = project(displayMatrix, cvPoint2D32f(corner.x,corner.y));
            cvCircle(out, cvPoint(display_corner.x, display_corner.y), 10, CV_RGB(0,0,255), 5, CV_AA,0);

             //printf("you are here:  %i : %i\n",gameData.coords[0].x, gameData.coords[1].x);
...
int main(int argc, char** argv) {
    ...
    boardLetter = argv[3][0]; //for the config files
    ...
    {
        sprintf(buf, "sidebarC.png", boardLetter);
    ...
    while(1) {
        ...
        cvShowImage( WND_MAIN, out);//(showPhotoFinish && photoFinish) ? photoFinish : out );
